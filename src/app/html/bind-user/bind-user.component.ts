...
export class BindUserComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.formModel.valid) {
      ...
      this.pay.GetWeixinBind(this.queryParams.openId, model.s_login_account, model.s_paddword).subscribe(res => {
        console.log(res);
        ...
        if (res.i_role_permission != 2 && res.i_role_permission != 0 && res.i_role_permission != 1) {
        ...
        if (jsonModel.i_role_permission == 0 || jsonModel.i_role_permission == 1) {
          this.router.navigate(['/main']);

        }
        else if (jsonModel.i_role_permission == 2) { //在p-home
