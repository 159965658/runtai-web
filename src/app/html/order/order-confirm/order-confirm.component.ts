...
export class OrderConfirmComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, private _router: Router, private _messageService: MessageService, private orderService: OrderService, private courseService: CourseService,
  ...
  ngOnInit() {
    //监控路由出口发出来的数据
    this.subscribeObject = this.courseService.getOrderConfirm().subscribe(res => {
      if (res == 1) {
        this.orderConfirm();
      }
    ...
    this.activaRouter.queryParams.subscribe(params => this.courseId = params.id);
    ...
    this.invPhone = this.fb.group({
      phone: [this.cacheUserModel.ChildPhone == "" ? this.cacheUserModel.s_parents_mob : this.cacheUserModel.ChildPhone, [Validators.required, PhoneValidator]],
    });
  }
  orderConfirm() {
    ...
    if (!this.invPhone.valid) {
      this._messageService.setMessage({ error: 'error', message: this.invPhone.controls.phone.errors.msg.info });
      return;
    }
